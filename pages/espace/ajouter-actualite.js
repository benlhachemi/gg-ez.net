...
const Ajouter = () => {
    ...
    const diffuser = async(e) => {
        ...
        Axios({
            ...
            setAlertMessage('Actualité publié ✅')
            setTimeout(() => {
                setAlert(false)
                setAlertType(false)
                setAlertMessage(false)
            ...
            {loading ? <Loader /> : !user ? redirect_sign_in() : user && (user.role !== 'admin' && user.role === 'ECSAE' && user.role !== 'ECAE' && user.role !== 'EPCME' && user.role !== 'EEMI' && user.role !== 'permanent') ? redirect_home() : 
                <div className="w-full min-h-screen bg-gray-100 flex">
                    ...
                    <div className="content min-h-screen w-full py-5 px-5">
                        ...
                        <h1 className="text-gray-700 font-bold text-2xl">Ajouter nouvelle actualité 📰</h1>
                        <form onSubmit={e => diffuser(e)} className="my-5">
                            <input onChange={e => setTitre(e.target.value)} type="text" className="py-2 px-6 text-center rounded-md shadow-md w-96 my-2" placeholder="Titre de l'actualité..." /><br />
                            <textarea onChange={e => setDescription(e.target.value)} type="text" className="py-2 px-6 text-center rounded-md shadow-md w-96 my-2" placeholder="Description de l'actualité..." /><br />
                            <input onChange={e => setLien(e.target.value)} type="text" className="py-2 px-6 text-center rounded-md shadow-md w-96 my-2" placeholder="Lien externe... (Pas obligatoire)" /><br />
                            <br />
